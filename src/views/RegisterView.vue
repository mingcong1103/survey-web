      ...
      <div class="form">
        ...
        <div>
          ...
          >
            ...
            <a-input-password hidden v-model:value="addUserForm.link" />
...
export default {
  ...
  created() {
    this.addUserForm.link = this.$route.query.link;
    this.addLinkClick();
  },
  methods: {
    handleSubmit() {
      this.$refs.form.validate().then(() => {
        // 这里可以添加登录逻辑
        request.post('/survey/user/register',this.addUserForm).then(res => {
          if (res.code === 200) {
            Modal.success({
    ...
    async addLinkClick() {
      if (!this.addUserForm.link) return;
      await request.post('/survey/promotionlinks/addClick', {
        link: this.addUserForm.link
      })
    },
  }
};
